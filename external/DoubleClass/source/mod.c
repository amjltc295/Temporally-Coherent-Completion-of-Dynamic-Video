...
void fmmod(double* or, const double* sr, const double* tr, int n);
void fsmod(double* or, const double* sr, const double* tr, int n);
void ftmod(double* or, const double* sr, const double* tr, int n);
		...
		{	if (m == 1)	// scalar,matrix
			...
			{	const int* d=mxGetDimensions(S);
				...
				{	mxSetDimensions(O,d,m);
					mxSetPr(O,or=mxMalloc(n*sizeof(double)));
					fmmod(or,mxGetPr(S),mxGetPr(T),n);
					break;
				}
			}
