...
numPolygon = 1;
for i = 1:numPolygon
    [mask, poly] = getMask(videoFrame);
    if(i == 1)
        holeMask = mask;
    else
        holeMask = holeMask | mask;
    end
end

% Visualization 
holeMaskEdge = edge(holeMask);

videoFrameR = videoFrame(:,:,1);
videoFrameG = videoFrame(:,:,1);
videoFrameB = videoFrame(:,:,1);

videoFrameR(holeMaskEdge) = 255;
videoFrameG(holeMaskEdge) = 0;
videoFrameB(holeMaskEdge) = 0;

videoFrameVis = cat(3, videoFrameR, videoFrameG, videoFrameB);

figure(2), imshow(videoFrameVis);
