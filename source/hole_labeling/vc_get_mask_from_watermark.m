function vc_get_mask(videoName)

videoName = 'VSB_planet_earth_2';

vidObj = VideoReader(fullfile('dataset', 'video', [videoName, '.avi']));
videoFrame = read(vidObj, 1);

holeMaskName = fullfile('dataset', 'hole', [videoName, '_hole.png']);

logoMask = imread('siggraph_logo.png');
logoMask = logoMask(500:end,300:end);
s = 0.5;
...
% Mask image width
maskWidth = s*imgW;
logoMaskCur = imresize(logoMask, [NaN, maskWidth]);
[maskH, maskW] = size(logoMaskCur);

% Create hole image
holeMask = zeros(imgH, imgW);
c = [2*imgH/3, imgW/2];
p = round(c - [maskH, maskW]/2); % top-left corner
holeMask(p(1):p(1)+maskH-1, p(2):p(2)+maskW-1) = im2double(logoMaskCur);
holeMask = im2bw(holeMask);

figure(2), imshow(holeMask);

imwrite(holeMask, holeMaskName);
