...
% Video file names
videoExt = 'avi';
videoResBName = fullfile(videoResPath, [videoName, '_b.', videoExt]);
videoResName  = fullfile(videoResPath, [videoName, '.',   videoExt]);

% Font size
fontSizeRatio = 0.1;
fontSize = round(size(videoData, 2) * fontSizeRatio);
...
open(wVidObj);
if(maskFlag)
%     wVidObjB = VideoWriter(videoResBName, 'MPEG-4');
    wVidObjB = VideoWriter(videoResBName);
    wVidObjB.Quality = 100;
    open(wVidObjB);
end

% Converting frame rate by duplication
FrameRate      = 5;
frameRateRatio = 30/FrameRate;
...
for i = 1: size(videoData, 4)
    ...
    % Overlay mask
    if(maskFlag)
        maskBd = bwperim(holeMask(:,:,i));
        imgB   = imoverlay(img, maskBd, [1, 0.25, 0.25]);
    end
    ...
    % Write video frames
    for j = 1: frameRateRatio
        writeVideo(wVidObj,   img);
        if(maskFlag)
            writeVideo(wVidObjB, imgB);
        end
    end
end
